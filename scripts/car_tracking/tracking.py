...
def get_correct_index(bag):
	...
	for topic, msg, t in bag.read_messages(topics='/imu/data', start_time=rospy.Time(bag.get_start_time())+rospy.Duration(0)):
...
def car_tracking(bag):
	...
	for topic, msg, t in bag.read_messages(topics=['/imu/data', '/scan'],
		...
		if topic == '/imu/data':
			...
			cloud_pcl.from_array(to_pcl_array(msg))
			if dt != 0:
				if dt > 1:
					queued_pcl_list = []
				else:
...
bag = rosbag.Bag('/home/feuerblitz7/bagfiles/marco_torso/test.bag', 'a')
